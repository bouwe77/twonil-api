...
namespace TwoNil.Logic.Competitions
{
    public class LeagueTableManager
    {
        ...
        public LeagueTableManager(IUnitOfWorkFactory uowFactory)
        {
        ...
        public void UpdateLeagueTable(LeagueTable leagueTable, IEnumerable<Match> matches)
        {
            foreach (var match in matches)
            {
                ...
                var homeTeam = leagueTable.LeagueTablePositions.First(pos => pos.TeamId == match.HomeTeamId);
                var awayTeam = leagueTable.LeagueTablePositions.First(pos => pos.TeamId == match.AwayTeamId);
            ...
            leagueTable.LeagueTablePositions.Sort((pos1, pos2) =>
            {
                ...
                if (result == 0)
                {
                    ...
                    {
                        result = CheckMatchResults(leagueTable, pos1, pos2);
                    }
                    else
                    {
        ...
        private int CheckMatchResults(LeagueTable leagueTable, LeagueTablePosition pos1, LeagueTablePosition pos2)
        {
            int result = 0;
            ...
            {
                matches = uow.Matches.GetMatchesBetweenTeams(leagueTable.SeasonCompetition, team1, team2).ToList();
            }
