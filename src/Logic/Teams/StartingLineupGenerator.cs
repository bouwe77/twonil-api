...
namespace TwoNil.Logic.Teams
{
    public class StartingLineupGenerator
    {
        ...
        public StartingLineupGenerator(IUnitOfWorkFactory uowFactory)
        {
            ...
            using (var uow = _uowFactory.Create())
            {
                ...
                {
                   { "Goalkeeper", new List<Position> {uow.Positions.GetGoalkeeper()} },
                   { "Centre Back", new List<Position> {uow.Positions.GetWingBack(), uow.Positions.GetDefensiveMidfield()} },
                   { "Wing Back", new List<Position> {uow.Positions.GetCentreBack(), uow.Positions.GetDefensiveMidfield()} },
                   { "Defensive Midfield", new List<Position> {uow.Positions.GetWingBack(), uow.Positions.GetCentreBack()} },
                   { "Central Midfield", new List<Position> {uow.Positions.GetWideMidfield(), uow.Positions.GetForwardMidfield(), uow.Positions.GetDefensiveMidfield()} },
                   { "Wide Midfield", new List<Position> {uow.Positions.GetCentralMidfield(), uow.Positions.GetDefensiveMidfield(), uow.Positions.GetForwardMidfield()} },
                   { "Forward Midfield", new List<Position> {uow.Positions.GetCentralMidfield(), uow.Positions.GetCentreForward(), uow.Positions.GetWinger()} },
                   { "Winger", new List<Position> {uow.Positions.GetForwardMidfield(), uow.Positions.GetCentreForward()} },
                   { "Centre Forward", new List<Position> {uow.Positions.GetForwardMidfield(), uow.Positions.GetStriker(), uow.Positions.GetWinger()} },
                   { "Striker", new List<Position> {uow.Positions.GetCentreForward(), uow.Positions.GetWinger(), uow.Positions.GetForwardMidfield()} }
                };
            }
        }
        ...
        public List<Player> GenerateStartingLineup(List<Player> players, Formation formation)
        {
            ...
            using (var uow = _uowFactory.Create())
            {
                goalkeeperPosition = uow.Positions.GetGoalkeeper();
            }

            var goalkeepers = players.Where(player => player.PreferredPositionId == goalkeeperPosition.Id).ToList();
            var bestFieldPlayers = players.Where(player => player.PreferredPositionId != goalkeeperPosition.Id).Take(10).ToList();
            var otherFieldPlayers = players.Where(player => player.PreferredPositionId != goalkeeperPosition.Id).Skip(10).ToList();
            ...
            while (teamOrder < 11)
            {
                ...
                var playerForPos = bestFieldPlayers.FirstOrDefault(player => player.PreferredPositionId == positionWeNeed.Id);
            ...
            for (int i = 0; i < 11; i++)
            {
                if (newPlayers[i] == null)
                {
                    ...
                    {
                        playerForPos = bestFieldPlayers.FirstOrDefault(player => player.PreferredPositionId == alternativePosition.Id);
                        if (playerForPos != null)
                        {
                            playerForPos.CurrentPosition = positionWeNeed;
                    ...
                    if (playerForPos == null)
                    {
                        playerForPos = otherFieldPlayers.FirstOrDefault(player => player.PreferredPositionId == positionWeNeed.Id);
                        if (playerForPos != null)
                        {
                            playerForPos.CurrentPosition = positionWeNeed;
                            playerForPos.TeamOrder = i;
                    ...
                    if (playerForPos == null)
                    {
                        foreach (var alternativePosition in alternativePositions)
                        {
                            playerForPos = otherFieldPlayers.FirstOrDefault(player => player.PreferredPositionId == alternativePosition.Id);
                            if (playerForPos != null)
                            {
                                playerForPos.CurrentPosition = positionWeNeed;
                                playerForPos.TeamOrder = i;
